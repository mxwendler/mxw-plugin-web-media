...
class web_instance:
    def __init__(self):
        ...
        self.browser = None
        self.page = None
        # latest decoded BGRA frame, produced by the worker thread and handed back to
        # the render thread in onRenderFrame. guarded by 'lock'.
        self.last_frame = None
        self.error = None
        self.lock = threading.Lock()
        # background worker that owns Playwright and screenshots the page off the
        # render thread, so web rendering never stalls the main application.
        self.thread = None
        self.stop_event = threading.Event()
        # navigation / resize requests handed to the worker (Playwright is thread-affine)
        self.commands = queue.Queue()
        self.grab_interval = 1.0 / 10.0   # re-grab at most ~10x / second
...
def _worker(inst, start_url, width, height):
    # background thread: owns the ENTIRE Playwright lifecycle (the sync API is
    # thread-affine) and grabs screenshots off the render thread. it never touches
    # mxw.* (not thread-safe) - it only writes inst.last_frame under inst.lock, which
    # onRenderFrame reads. all the slow work (launch, goto, screenshot) lives here, so
    # the main application is never blocked by web page rendering.
    try:
        ...
        inst.page.goto(start_url, wait_until="domcontentloaded", timeout=20000)
    except Exception as e:
        # no synchronous onOpen to report to anymore; stash the cause and bail out.
        # the clip simply shows blank frames.
        with inst.lock:
            inst.error = str(e)
        _worker_cleanup(inst)
        return

    while not inst.stop_event.is_set():
        # apply queued navigation / resize requests (must run on this thread)
        while True:
            try:
                cmd = inst.commands.get_nowait()
            except queue.Empty:
                break
            try:
                if cmd[0] == "goto":
                    inst.page.goto(cmd[1], wait_until="domcontentloaded", timeout=20000)
                elif cmd[0] == "resize":
        ...
        # so the render thread keeps running while we wait.
        try:
            ...
            if bgra.shape[1] != width or bgra.shape[0] != height:
                bgra = cv2.resize(bgra, (width, height))
            # the screenshot is top-down (row 0 = top), but the stream texture treats
            # row 0 as the bottom (GL origin is bottom-left) -> flip
            frame = np.ascontiguousarray(np.flipud(bgra))
            with inst.lock:
                inst.last_frame = frame
        except Exception:
            pass

        # throttle, and wake immediately on stop so onClose joins quickly
        inst.stop_event.wait(inst.grab_interval)

    _worker_cleanup(inst)


def _worker_cleanup(inst):
    # close Playwright on its OWN thread; the main thread must never do this.
    try:
...
def _goto(inst, url):
    # hand the navigation to the worker thread (Playwright is thread-affine); keep the
    # field text in sync so the panel shows the (normalized) url we are loading.
    if not url:
        return
    with inst.lock:
        inst.url = url
    inst.commands.put(("goto", url))
...
def onOpen(uri):
    ...
    # launch the browser and grab frames on a background thread: onOpen returns
    # immediately with our fixed surface traits, so neither the page load nor the
    # one-time chromium download blocks the application.
    inst.thread = threading.Thread(
        target=_worker, args=(inst, url, inst.width, inst.height), daemon=True)
    inst.thread.start()
...
def onRenderFrame(frame):
    # the render thread does NO blocking work here: just hand back the latest frame
    # the worker thread produced (or a blank until the first screenshot arrives).
    inst = storage.get(media_id)
    if inst is None:
        return _blank(1280, 720)
    with inst.lock:
        if inst.last_frame is not None:
            return inst.last_frame
        return _blank(inst.width, inst.height)


def onSave():
    # persist the loaded url so it is restored on reload. the host stores the returned
    # string in the project and hands it back to onLoad().
    inst = storage.get(media_id)
    if inst is None:
        return None
    with inst.lock:
...
def onLoad(state):
    ...
    # navigate to the saved url (queued for the worker thread).
    inst = storage.get(media_id)
    if inst is None or not state:
        return
    try:
...
def onSizeChange(w, h):
    # the host changed our render size: record it for _blank() and queue a viewport
    # resize for the worker thread (which actually drives the browser).
    inst = storage.get(media_id)
    if inst is None:
        return
    with inst.lock:
        inst.width = int(w)
        inst.height = int(h)
    inst.commands.put(("resize", int(w), int(h)))
...
def onClose():
    ...
    if inst is None:
        return
    # stop the grab loop; the worker closes Playwright itself, on its own thread.
    inst.stop_event.set()
    if inst.thread is not None:
        inst.thread.join(timeout=10.0)
